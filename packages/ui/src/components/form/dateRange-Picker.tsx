...
export function DateRangePicker({
  ...
  const initialMonth = isDateRange(dateValue) ? dateValue.from : dateValue;
  ...
  const singleDate = isDateRange(dateValue) ? dateValue?.from : dateValue;
  ...
  const handlePresetSelect = (date: Date | DateRange) => {
          ...
          >
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <span className="truncate">
                {isDateRange(dateValue) && dateValue.from ? (
                  <span className="flex items-center gap-1 whitespace-nowrap">
                    <span>{format(dateValue.from, "PPP")}</span>
                    {dateValue.to && (
                      <>
                        <span> - </span>
                        <span>{format(dateValue.to, "PPP")}</span>
                      </>
                    )}
                    {dateValue.to && isToday(dateValue.to) && (
            ...
            <div className="flex items-center gap-1">
              {dateValue && !disabled && (
                <span
                  ...
                  className="h-4 w-4 rounded-full hover:bg-muted flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                >
                  <X className="h-3 w-3" />
                </span>
              )}
