import { IJobPositionCreateDtoType, IJobPositionUpdateDtoType } from "@skillsmatch/dto";
...
export const CreateJobPosition = async (data: IJobPositionCreateDtoType) => {
    await ensureUniqueRecord({ table: "jobPosition", column: "name", value: data.name });
...
export const UpdateJobPosition = async (id: string, data: IJobPositionUpdateDtoType) => {
    if (data.name) await ensureUniqueRecord({ table: "jobPosition", column: "name", value: data.name });
