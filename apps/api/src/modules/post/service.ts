  ...
  try {
    const post = await prisma.$transaction(async (tx) => {
      ...
      const postCourseData =
        data.courseIds?.map((item) => ({ pId: post.id, crId: item })) ?? [];
      await tx.postCourse.createMany({
        data: postCourseData,
      });
      const postEducationInstitutionData =
        data.educationInstitutionIds?.map((item) => ({
          pId: post.id,
          eiId: item,
        })) ?? [];
      await tx.postEducationInstitution.createMany({
        data: postEducationInstitutionData,
      });
      const postEducationLevelData =
        data.educationLevelIds?.map((item) => ({ pId: post.id, elId: item })) ??
        [];
      ...
      await Promise.all(
        (data.jobPositions ?? []).map(async (element) => {
          ...
          const pjpdData =
            (await element.skillIds?.map((item) => ({
              pjpId: pjpd.id,
              skId: item,
            }))) ?? [];
          const a = await tx.postJobPositionDetailSkill.createMany({
            data: pjpdData,
          });
        })
      );
...
export const GetPostUpdate = async (id: string) => {
  ...
  return {
    ...
    educationInstitutionIds: post.postEducationInstitution.map(
      (pei) => pei.eiId
    ),
