...
export const ReportPrintAreaPost = React.forwardRef<HTMLDivElement>(
  (props, ref) => {
    const {
      statsQuery: {
        data: statusData,
    ...
    return (
      <div ref={ref} className="p-6 bg-white text-black max-w-full">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold mb-2">Job Position Report</h1>
        ...
        <div>
            ...
            <table className="w-full border-collapse bg-white">
              <thead>
                <tr className="bg-gray-100 border-b-2 border-gray-200">
                  ...
                  <th className="p-4 text-left text-gray-700">Total Usage</th>
                </tr>
              </thead>
              <tbody>
                {tableQueryMosJobposition?.data.map((data, idx) => (
                  <tr
                    key={idx}
          ...
          {(!tableQueryMosJobposition?.data ||
            tableQueryMosJobposition.data.length === 0) && (
            <div className="text-center py-8 text-gray-500">
              <p>No Most JobPosition found in the database.</p>
            </div>
          )}
        </div>
