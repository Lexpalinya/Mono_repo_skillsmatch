...
interface FetchAllJobPositionParams {
...
export const fetchAllJobPosition = async ({
  pagination,
  globalFilter,
  columnFilters,
  sorting,
}: FetchAllJobPositionParams) => {
  ...
  const result = await trpcClient.jobPosition.getAll.query(queryParams);
