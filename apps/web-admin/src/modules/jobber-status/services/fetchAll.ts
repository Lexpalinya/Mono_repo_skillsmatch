...
interface FetchAllJobberStatusParams {
...
export const fetchAllJobberStatus = async ({
  pagination,
  globalFilter,
  columnFilters,
  sorting,
}: FetchAllJobberStatusParams) => {
  ...
  const result = await trpcClient.jobberStatus.getAll.query(queryParams);
